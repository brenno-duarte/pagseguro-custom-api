...
# VOCÊ PODE EXCLUIR ESTAS VARIÁVEIS EM UM PROJETO REAL
...
<script src="<?= PAGSEGURO_SCRIPT ?>"></script>
...
<script>
    ...
    // ALTERA O FORMULÁRIO EXIBINDO E OCULTANDO OS DADOS DE CARTÃO E BOLETO
    ...
    // ALTERA ENTRE OS DADOS DO MESMO COMPRADOR E OUTRO COMPRADOR
    ...
    // REINSERE OS DADOS SE A OPÇÃO "MESMO QUE O COMPRADOR" FOR MARCADA
    function ReInserir() {
    ...
    // CONSULTA AS PARCELAS DÍSPONIVEIS
    ...
    // VERIFICA SE O CARTÃO É VÁLIDO E EXIBE A BANDEIRA DO CARTÃO
    function brandCard() {
    ...
    // SCRIPT DE PAGAMENTO COM BOLETO
    function pagarBoleto() {
        $("#warning-error").html("Por favor aguarde...");
        ...
        $.ajax({
            ...
            data: {
                id: $("#idProduct").val(),
                desc: $("#descProduct").val(),
                valor: $("#amountProduct").val(),
            ...
            success: function(data) {
                if (!(data.paymentLink)) {
                    ...
                    $("#warning-error").html("Pagamento efetuado com sucesso!!! Consulte o seu Sandbox do PagSeguro");

                    /* 
                    
                    OU ENTÃO UTILIZE O SCRIPT ABAIXO PARA FAZER O REDIRECIONAMENTO
                    
                    window.location = data.paymentLink;
                    setTimeout(function() {
                        $("#warning-error").html("Caso você não seja redirecionado para o seu boleto, clique no botão abaixo.<br /><br /><a href='" + data.paymentLink + "'><center><img src='images/boleto.png' /><br /><br /><button class='btn-success btn-block btn-lg'>Ir para o meu boleto</button></center></a>");
                    }, 3500); 
    ...
    // SCRIPT DE PAGAMENTO COM CARTÃO DE CRÉDITO
                ...
                if (data.error) {
                    ...
                    $.ajax({
                        ...
                        success: function(status) {
                            ...
                            if (status == "7") {
                                alert(status);
                                $("#warning-error").html("Erro ao processar o seu pagamento.<br/> Não se preocupe pois esse valor <b>não será debitado de sua conta ou não constará em sua fatura</b><br /><br />Verifique se você possui limite suficiente para efetuar a transação e/ou tente um cartão diferente");
                            } else {
                                $("#warning-error").html("Pagamento efetuado com sucesso!!! Consulte o seu Sandbox do PagSeguro");
                                ...
                                setTimeout(function() {
                                    $("#warning-error").html("Caso você não seja redirecionado para a nossa página de instruções, clique no botão abaixo.<br /><br /><a href='http://download.infoenem.com.br/pagamento-efetuado/'><center><button class='btn-success btn-block btn-lg'>Ir para a página de instruções</button></center></a>");
                                }, 1500); 
    ...
    // EXIBE OS ERROS DO FORMULÁRIO AO ENVIAR PARA O PAGSEGURO
    function tratarError(id) {
        if (id.charAt(0) == '2') id = id.substr(1);
        if (id == "53020" || id == '53021') {
            $("#warning-error").append("<p>Verifique telefone inserido</p>");
            $("#senderPhone").css('border', '2px solid red');

        } else if (id == "53010" || id == '53011' || id == '53012') {
            $("#warning-error").append("<p>Verifique o e-mail inserido</p>");
            $("#senderEmail").css('border', '2px solid red');

        } else if (id == "53017") {
            $("#warning-error").append("<p>Verifique o CPF inserido</p>");
            $("#senderCPF").css('border', '2px solid red');

        } else if (id == "53018" || id == "53019") {
            $("#warning-error").append("<p>Verifique o DDD inserido</p>");
            $("#senderAreaCode").css('border', '2px solid red');

        } else if (id == "53013" || id == '53014' || id == '53015') {
            $("#warning-error").append("<p>Verifique o nome inserido</p>");
            $("#senderName").css('border', '2px solid red');

        } else if (id == "53029" || id == '53030') {
            $("#warning-error").append("<p>Verifique o bairro inserido</p>");
            $("#shippingAddressDistrict").css('border', '2px solid red');

        } else if (id == "53022" || id == '53023') {
            $("#warning-error").append("<p>Verifique o CEP inserido</p>");
            $("#shippingAddressPostalCode").css('border', '2px solid red');

        } else if (id == "53024" || id == '53025') {
            $("#warning-error").append("<p>Verifique a rua inserido</p>");
            $("#shippingAddressStreet").css('border', '2px solid red');

        } else if (id == "53026" || id == '53027') {
            $("#warning-error").append("<p>Verifique o número inserido</p>");
            $("#shippingAddressNumber").css('border', '2px solid red');

        } else if (id == "53033" || id == '53034') {
            $("#warning-error").append("<p>Verifique o estado inserido</p>");
            $("#shippingAddressState").css('border', '2px solid red');

        } else if (id == "53031" || id == '53032') {
            $("#warning-error").append("<p>Verifique a cidade informada</p>");
            $("#shippingAddressCity").css('border', '2px solid red');

        } else if (id == '10001') {
            $("#warning-error").append("<p>Verifique o número do cartão inserido</p>");
            $("#cardNumber").css('border', '2px solid red');

        } else if (id == '10002' || id == '30405') {
            $("#warning-error").append("<p>Verifique a data de validade do cartão inserido</p>");
            $("#cardExpirationMonth").css('border', '2px solid red');
            $("#cardExpirationYear").css('border', '2px solid red');

        } else if (id == '10004') {
            $("#warning-error").append("<p>É obrigatorio informar o código de segurança, que se encontra no verso, do cartão</p>");
            $("#cardCvv").css('border', '2px solid red');

        } else if (id == '10006' || id == '10003' || id == '53037') {
            $("#warning-error").append("<p>Verifique o código de segurança do cartão informado</p>");
            $("#cardCvv").css('border', '2px solid red');

        } else if (id == '30404') {
            $("#warning-error").append("<p>Ocorreu um erro. Atualize a página e tente novamente!</p>");

        } else if (id == '53047') {
            $("#warning-error").append("<p>Verifique a data de nascimento do titular do cartão informada</p>");
            ...
            $("#warning-error").append("<p>Verifique o código de área informado</p>");
            $("#creditCardHolderAreaCode").css('border', '2px solid red');

        } else if (id == '53122') {
            $("#warning-error").append("<p>Enquanto na sandbox do PagSeguro, o e-mail deve ter o domínio '@sandbox.pagseguro.com.br' (ex.: comprador@sandbox.pagseguro.com.br)</p>");

        }

        // else {
        //   $("#warning-error").append("<p>"+ id + "</p>");
        // }
    }
</script>